...
if($_GET['filter'] != 'filter')
{
 ...
 if($this->session->userdata('user_id')=='1')
	...
	{
		$qry = "select * from tbl_stock_in_hdr H, tbl_stock_in_inv_hdr I where H.stockinid = I.stockinhdr AND H.order_type != 'other_inbound' AND H.comp_id='".$this->session->userdata('comp_id')."' order by I.inv_no desc ";
		$qry1= "select * from tbl_stock_in_hdr H, tbl_stock_in_dtl D where H.stockinid = D.stockinhdr AND H.order_type != 'other_inbound' AND H.comp_id='".$this->session->userdata('comp_id')."' order by I.inv_no desc " ;
	}
