...
if($poID!="")
{
$invice12=$this->db->query("select * from tbl_purchase_order_hdr where purchaseorderid='".$_GET['id']."'");
...
<?php
$Query=$this->db->query("select * from tbl_purchase_order_hdr where vendor_id='".$_GET['contact_id']."' ");
foreach($Query->result() as $getPo)
  {
  ?>

<li class="active">
<a href="<?=base_url();?>purchaseorder/purchaseorder/purchase_order_details?id=<?=$getPo->purchaseorderid;?>&&contact_id=<?=$getPo->vendor_id;?>"><i class="fa fa-inbox"></i> <?=$getPo->invoice_date; ?><b>(<?=$getPo->purchaseorderid;?>)</b></a>					
</li>
<?php }?>
</ul>
...
<a href="<?=base_url();?>NewPayment/payment_amount_po?id=<?php echo $_GET['id'];?>&&contact_id=<?php echo $_GET['contact_id'];?>" class="btn btn-primary" target="_blank">Record Payment</a><button class="btn btn-primary" type="button" onClick="openpopup('<?=base_url();?>purchaseorder/purchaseorder/edit_purchase_order',1200,500,'id',<?php echo $_GET['id'];?>)">Edit Purchase Order</button><button style="display:none" class="btn btn-primary" type="button" onClick="openpopup('<?=base_url();?>purchaseorder/purchaseorder/salesOrder_details_mail',1200,500,'id',<?php echo $_GET['id'];?>)">Mail Purchase Order</button></div>
</div>
</div><!--row-->
<br>
<?php 
$softwareLog=$this->db->query("select *from tbl_software_log where contact_id='".$_GET['contact_id']."' and log_id='".$_GET['id']."' and type='Purchase Order added' order by id desc");
foreach($softwareLog->result() as $getSoftware){
?>

<div class="row">
<div class="col-sm-3">
<?=$getPo->invoice_date;?>&nbsp;<?=$getSoftware->author_id;?>
</div>
<div class="col-sm-9">
...
<?php
} elseif($getSoftware->type=='Purchase Order added'){
?>
Po No. <?=$getSoftware->log_id?> of amount ₹ <?=$getSoftware->total; if($getSoftware->type=='Purchase Order added'){ ?> created by <?php }else{ echo "update by"; } ?> <?php echo $this->session->userdata('user_name');?> <a style="color:#ec407a" href='#' onclick="openpopup('<?=base_url();?>purchaseorder/purchaseorder/edit_purchase_order',1200,500,'view',<?=$getSoftware->log_id;?>)">View</a> 
<?php
}
else
{}
...
<br>
<strong>Rs <?=$getPayment= $obj->pur_payment_due($_GET['contact_id']);?></strong>
</div>
</div>
</div>
...
<table id="meta">
<tr>
<td class="meta-head">Purchase Order Date :</td>
...
<tr>
<td class="meta-head" style="display:none">Due Date :</td>
<td style="display:none"><div class="due"><?php 
...
extract($_POST);
$n=1;
$inviceQ=$this->db->query("select * from tbl_purchase_order_dtl where purchaseorderhdr='$poID'");
...
<td class="total-value balance"><div class="due"><strong>Rs <?=$getPayment= $obj->pur_payment_due($_GET['contact_id']);?></strong></div></td>
</tr>
</table>
<div id="height"></div>
<div id="terms">
