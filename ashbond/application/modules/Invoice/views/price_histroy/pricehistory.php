...
<td width="19%" class="text-r">
Invoive. No.:</td>
 <td width="33%"><input type="text" name="invno"   id="id3" onKeyUp="search_row(this.id,4)" /></td>
<td width="19%" class="text-r">
Price List:</td>
<td width="29%"><input type="text" name="price_list"  id="id4" onKeyUp="search_row1(this.id,5)" /></td></tr>     
<tr>
...
@extract($_POST);
	...
	if(@$_GET['popup']=='True') {
		...
		if($invno!=''){
				 $queryy.=" and salesid='$invno'";
		}
		if($price_list!=''){
				 $queryy.=" and list_price='$price_list'";
		}
	}
	if(@$_GET['popup']=='True') {
  ...
  <?php echo $getProduct->productname;?></td>
	 <?php $hdrQuery=$this->db->query("select *from tbl_sales_order_hdr where salesid='$line->salesid'");
    ...
    <div class="modal-content" style="width:1210px;height:200px;; margin-left:-302px;">
	 ...
	 <strong>List Price</strong>
	  ...
	  <?php
						...
						$sqlddd = $this->db->query("select * from tbl_contact_m where status ='A' and contact_id='$getProduct->vendor_id'");
