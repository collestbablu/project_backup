...
$selQuery=$this->db->query("select * from tbl_invoice_payment where contact_id='$contact' and comp_id='".$this->session->userdata('comp_id')."' and status='payment'");

$fetchselQ=$selQuery->row();

if($fetchselQ->contact_id!='' && $fetchselQ->status=='payment'){
if($date123==''){
$date123=date('d-m-y');}

$this->db->query("update tbl_invoice_payment set receive_billing_mount=receive_billing_mount+'$rec_amount12',pay_modes='$pay_modes',date='$date123' where contact_id='$contact' and comp_id='".$this->session->userdata('comp_id')."' and status='payment'");
	
	}else{
	
	if($date123==''){
$date123=date('d-m-y');}
	$sqlinsert="insert into tbl_invoice_payment set contact_id='$customerfname',receive_billing_mount='$rec_amount12',pay_modes='$pay_modes',date='$date123',maker_id='".$this->session->userdata('user_id')."',maker_date=NOW(),comp_id='".$this->session->userdata('comp_id')."', status='payment'";
...
//redirect('Payment/payment/payment_amount');
}
...
<option value="">---select---</option>
 <?php
 $contQuery=$this->db->query("select * from tbl_contact_m where comp_id='".$this->session->userdata('comp_id')."' and group_name='3' order by first_name");
...
<th width="7%"></th> 
	...
	if($Search!='' || $_GET['id']!='')
	{
		 ...
		 $queryy="select * from $tableName where status='A' and vendor_id='$customerfname' and comp_id = '".$this->session->userdata('comp_id')."'";
...
$total_bill=0;
 $queryy1="select * from tbl_invoice_payment where status='purchaseorder' and contact_id='$customerfname' and comp_id = '".$this->session->userdata('comp_id')."'";
...
?>
<tr>
<td width="20%" class="text-r">Total Amount : </td>
<td width="30%"><input type="text" id="total_billamt" name="total_billamt" size="10" value="<?php echo $total_bill; ?>" readonly /></td>
<td class="text-r">Amount : </td><td><input type="hidden" id="total_billamt_receive" name="total_billamt_receive" value="<?php if($total_billamt!=''){ echo $rowin1->total_billamt_receive;} ?>" onchange="receivealert()" size="10" />
...
<?php if($customerfname!=''){ ?>
 <a href="#" onclick="openpopup('invoicereport',900,630,'id','<?php echo $customerfname;?>')" >Payment Amount</a>
<?php }else{ ?>
Payment Amount
<?php } ?>
</td>
...
<td><input type="date" name="date123" value="" /></td>
</tr> 
<tr>
...
<td><select name="pay_modes">
	<option value="">--select--</option>
<?php
$querypayModel=$this->db->query("select * from tbl_invoice_payment where status='payment' and contact_id='".$_GET['id']."' and comp_id = '".$this->session->userdata('comp_id')."'");
