...
<table class="bordered" id="dataTables-example">
	...
	<?php if($_GET['show_row']!=''){
						...
						$sql1 = $this->db->query("select * from production_production_dtl where invoice_id='".$_GET['show_row']."'");
					   			foreach($sql1->result() as $fetch){
								//echo "select * from tbl_product_stock where product_id='$fetch->product_id'"; 
								
							
							$Query=$this->db->query("select * from tbl_product_stock where product_id='$fetch->product_id'");
						    $fetchQ=$Query->row();?>
								<tr>
