...
@extract($_POST);
	...
	{
	if(@$_GET['popup']=='True') {
		$queryy="select * from $tableName where status='A' and vendor_id='".$_GET['id']."' order by quotation_id_dtl desc";
		}
		else
		{
		 $queryy="select * from $tableName where status='A' order by quotation_id_dtl desc";
		}
		if($vendor_id!='')
		{
		...
		if($quation_no!=''){
				 $queryy.=" and quotation_id ='$quation_no'";
		}
		if($list_price!=''){
   ...
   <tr>
   	<td><?php echo $line->quotation_id;?></td>
	<td>
  ...
  <?php $hdrQuery=$this->db->query("select *from tbl_quotation_hdr where quotation_id_hdr='$line->quotation_id'");
