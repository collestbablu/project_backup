...
(/** @lends <global> */function( $ ) {
	...
	function _fnReDraw( settings, holdPosition )
	{
		...
		if ( filter ) {
			_fnFilterComplete( settings, settings.oPrevSearch );
		}
		else {
	...
	function _fnAjaxParameters( settings )
	{
		var
			...
			features = settings.oFeatures,
			preSearch = settings.oPrevSearch,
	...
	function _fnFeatureHtmlFilter ( settings )
	{
		...
		var PrevSearch = settings.oPrevSearch;
		...
		var jqFilter = $('input[type="search"]', filter)
			.val( PrevSearch.sSearch.replace('"','&quot;') )
			.bind( 'keyup.DT search.DT input.DT paste.DT cut.DT', function(e) {
				...
				/* Now do the filter */
				if ( val != PrevSearch.sSearch ) {
					_fnFilterComplete( settings, {
						"sSearch": val,
						"bRegex": PrevSearch.bRegex,
						"bSmart": PrevSearch.bSmart ,
						"bCaseInsensitive": PrevSearch.bCaseInsensitive
					} );
		...
		$(settings.nTable).on( 'filter.DT', function () {
			...
			try {
				if ( jqFilter[0] !== document.activeElement ) {
					jqFilter.val( PrevSearch.sSearch );
				}
			}
			catch ( e ) {}
		} );
	...
	{
		var oPrevSearch = oSettings.oPrevSearch;
	...
	function _fnFilter( settings, input, force, regex, smart, caseInsensitive )
	{
		...
		var prevSearch = settings.oPrevSearch.sSearch;
	 ...
	 *  @param {object} settings DataTables settings object
	 *  @param {string|int} action Paging action to take: "first", "Prev",
	...
	function _fnScrollBarWidth ()
	{
		...
		// Subsequent runs will just use the Prevly calculated value
	...
	function _fnSaveState ( oSettings )
	{
		...
		var oState = {
			...
			"oSearch":      $.extend( true, {}, oSettings.oPrevSearch ),
	...
	function _fnLoadState ( oSettings, oInit )
	{
		...
		/* Search filtering  */
		$.extend( oSettings.oPrevSearch, oData.oSearch );
	...
	DataTable = function( options )
	{
		 ...
		 * Prev, first or last pages.
		 *  @param {string|int} mAction Paging action to take: "first", "Prev", "next" or "last"
		...
		this.each(function() {
			...
			for ( i=0, iLen=allSettings.length ; i<iLen ; i++ )
			{
				...
				/* If the element we are initialising has the same ID as a table which was Prevly
	...
	_api_register( 'search()', function ( input, regex, smart, caseInsen ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			_fnFilterComplete( settings, $.extend( {}, settings.oPrevSearch, {
				"sSearch": input+"",
	...
	_api_register( [
		...
		return this.iterator( 'column', function ( settings, column ) {
			...
			_fnFilterComplete( settings, settings.oPrevSearch, 1 );
		} );
	} );
	...
	DataTable.defaults = {
		 ...
		 * the page the display display will match what thy had Prevly set up.
		 *
		...
		"oLanguage": {
			...
			"oPaginate": {
				...
				/**
				 * Text to use for the 'Prev' pagination button (to take the user to
				 * the Prev page).
				 *  @type string
				 *  @default Prev
				 *
				 *  @dtopt Language
				 *  @name DataTable.defaults.language.paginate.Prev
				 *
				 *  @example
				 *    $(document).ready( function() {
				 *      $('#example').dataTable( {
		 ...
		 *
		 * * `simple` - 'Prev' and 'Next' buttons only
		 * * 'simple_numbers` - 'Prev' and 'Next' buttons, plus page numbers
		 * * `full` - 'First', 'Prev', 'Next' and 'Last' buttons
		 * * `full_numbers` - 'First', 'Prev', 'Next' and 'Last' buttons, plus
		 *   page numbers
		 *  
	...
	DataTable.ext = _ext = {
		 ...
		 * * `Prev` - Show Prev page when activated
		 ...
		 *
		 *  @example
		 *    // Show Prev, next and current page buttons only
