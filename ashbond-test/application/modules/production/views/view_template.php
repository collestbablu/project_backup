...
?>
<?php
$invoice_hdrQ=$this->db->query("select * from production_production_dtl where invoice_id='".$_GET['view']."'");
...
<?php
if(@$_GET['view']!=''){
@$branchQuery = $this->db->query("SELECT * FROM production_production_dtl where invoice_id='".$_GET['view']."'");
...
<td style="display:none"><input type="hidden" name="invoiceid" class="span6 " value="<?php echo $branchFetch->invoice_dtl_id;?>" readonly size="22" aria-required='true' /></td>
...
<form method="post" action="insert_sale">
<div style="width:100%; height:90%;  margin-top:2%;">
<nav class="navbar navbar-default">
 <div class="navbar-center navbar-brand" href="#"><a class="navbar-brand">View Production Template</a></div>
 <?php 
...
<input type="hidden" name="invoiceid1" id="invoiceid" value="<?php echo $invoiceID; ?>">
  ...
  <table id="hdr" data-uid="hdr"  class ="items" style="margin-top:1%; width:98%; margin-left:1%; margin-right:1%;" border="0" cellspacing="1" cellpadding="1">
    <tr>
			<td>Finish Goods</td>
			<td>
				<select name="finish_goods" <?php if($_GET['view']!=''){ ?> disabled="disabled" <?php } ?>>
...
$saleQ=$this->db->query("select * from production_production_dtl where invoice_id='".$_GET['view']."'");
...
<input name="item[]" id="item<?php echo $i;?>" style="border:hidden;width:100%; height:100%;" type="text"  onFocus="return data(this.id);textbox(this.id);" onKeyPress="getdata1();return get(this.id,(this.parentNode.parentNode.rowIndex));textbox(this.id);" osnKeyUp="return get(this.id,(this.parentNode.parentNode.rowIndex));textbox(this.id);" class="rounddefault-width-input" value="<?php if($saleFetch->product_id==$fetchSale->Product_id){ echo $fetchSale->productname; } else { echo $saleFetch->product_id; } ?>" readonly /></td>

<td><input name="qnty[]"  type="text" id="qnty<?php echo $i;?>"  maxlength="200"   class="round default-width-input my_with" onKeyPress="quantity_change(this.id);return isNumber(event);" onKeyUp="quantity_change(this.id);return isNumber(event);"<?php if($f==1) {?> onFocus="" <?php }?> style="border:hidden;width:100%; height:100%;" tabindex="-1"  value="<?php if($_GET['view']!=''){ echo $saleFetch->unit; } ?>" readonly />
<input type="hidden" size="10" id="hqnty<?php echo $i;?>" name="hqnty<?php echo $i;?>" value=""/>
</td>
<td class="total-line"><input type="text" name="unit[]" id="unit<?php echo $i;?>" style="border:hidden;width:100%; height:100%;" value="<?php if($_GET['view']!=''){ echo $saleFetch->qnty; } ?>" readonly /></td>
</tr>
<?php 
$sum=$sum+$i;
$i++;
...
<td style="border:hidden" align="right"><input type="hidden"  class="btn-success btn-sm" value="Save"/>&nbsp;&nbsp;<input id="aaaaa" class="btn-danger btn-sm " type="button"  onClick="cancelform()"   value="Cancel"/></td>
