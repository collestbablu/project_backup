...
public function addInvoiceNo(){
	if($this->session->userdata('is_logged_in')){
		$this->load->view('add-invoice-number');
	}
	else
	{
	redirect('index');
...
public function purchaseeignoress(){
	...
	$this->db->query("update tbl_purchase_order_hdr set ignores='ignore', ignores_date='$datet' where purchase_order_id='$id'");	
...
public function purchaseedismiss(){

		$id=$_GET['id'];
		
	$this->db->query("update tbl_purchase_order_hdr set dismiss='dismiss' where purchase_order_id='$id'");	
...
public function edititembyquotation(){
	if($this->session->userdata('is_logged_in')){
		$this->load->view('edit-item-by-purchase-order');
	}
	else
	{
	redirect('index');
...
public function editpurchaseorderitem(){
		...
		$this->db->query("update tbl_purchase_order_hdr set sub_total='$sub_name',grand_total='$grand_name'  where purchase_order_id='$quotation_id'");
		
		$this->db->query("update tbl_purchase_order_dtl set description='$desss',list_price='$sellingprice_name',quantity='$qty_name',per_discount='$disc_name',discountamount='$disc_amt_name',remark='$remark_name',total='$total_name',net_price='$net_name',sub_total='$sub_name',grand_total='$grand_name' where purchase_order_dtl_id='$quotation_id_dtl'");
		
		$this->db->query("update tbl_purchase_order_dtl set sub_total='$sub_name',grand_total='$grand_name' where purchase_order_hdr_id='$quotation_id'");
...
public function updateInvoice(){
				@extract($_POST);
		$table_name ='tbl_po_invoice_no';
		$pri_col ='po_invoice_id';
	 	$id= $this->input->post('po_iddd');
		...
		for ($i = 0; $i < $namecont; $i++) {
					...
					$con_id=$this->input->post('po_in_id')[$i];
					
					$queryconPer=$this->db->query("select * from tbl_po_invoice_no where po_id='$id' and po_invoice_id='$con_id'");
					$num= $queryconPer->num_rows();
					
					if($num >0)
				{
				
			$this->db->query("update tbl_po_invoice_no set po_id='$id', invoice_no='$val[$i]',invoice_date='$valemail[$i]',maker_id='".$this->session->userdata('user_id')."',maker_date='".$this->input->post('date_name')."',author_date='".date('y-m-d')."',comp_id='".$this->session->userdata('comp_id')."',zone_id='".$this->session->userdata('zone_id')."',brnh_id='".$this->session->userdata('brnh_id')."' where po_id='$id' and po_invoice_id='$con_id'");
					...
					$data_dtl['po_id']=$id;
					$data_dtl['invoice_no']=$val[$i];
					$data_dtl['invoice_date']=$valemail[$i];
...
$this->db->query("update tbl_communication set date='".$this->input->post('date')."',remark_name='".$this->input->post('remark_name')."',subject='".$this->input->post('subject')."' where communication_type='PurchaseOrder' and updateid='$id'");
			...
			$this->db->query("update tbl_purchase_order_hdr set delivery_date='".$this->input->post('date')."',reminder_date='".$this->input->post('reminder_date')."',currency_name='".$this->input->post('currency_name')."',remark='".$this->input->post('remark_name')."',contant='".$this->input->post('contant_name')."',subject='".$this->input->post('subject')."',sub_total='".$this->input->post('sub_total')."',service_charge_percentage ='".$this->input->post('service_chargeper')."',

					service_charge='".$this->input->post('service_charge')."',gross_discount_percentage='".$this->input->post('installation_chargeper')."',

					gross_discount='".$this->input->post('installation_charge')."',grand_total='".$this->input->post('nett')."',termandcondition='".$this->input->post('termandconditionedit')."',customer_dod='$customer_dod',vendor_dod='$vendor_dod',new_ref_no='$new_ref_no' where purchase_order_id='$id'");	
...
$this->db->query("update tbl_approve_status set dismiss_status='show' where order_id='$id'");
