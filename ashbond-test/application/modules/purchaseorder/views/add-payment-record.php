...
$location='PurchaseOrder/managePurchaseOrder';
...
$slectQuery1=$this->db->query("select * from tbl_invoice_report where invoiceid='$inv' and contact_id='$cont' and type='Purchase'");
...
<tr>
			...
			<td><input type="number" step="any" name="amount"  value="<?=$getTot1->billamount;?>" /></td>
   ...
   <td><input name="cid[]" type="checkbox" id="cid[]" class="sub_chk" data-id="<?php echo $line->invoice_id; ?>"  value="<?php echo $line->invoice_id?>" /></td>
					   ...
					   <td><?php echo $line->date; ?></td>
                         ...
                         <td><?php echo $line->billamount;?></td>
                            ...
                            <td><a href="<?=base_url();?>purchaseorder/PurchaseOrder/addPayment?amt=<?=$_GET['amt'];?> && id=<?=$line->invoice_rid;?> ">Edit</a></td>
...
<a href="javascript:window.close();" title="Cancel" class="submit"> Cancel</a>
       ...
       <a href="<?=base_url();?>purchaseorder/PurchaseOrder/managePurchaseOrder" title="Cancel" class="submit"> Cancel</a>
