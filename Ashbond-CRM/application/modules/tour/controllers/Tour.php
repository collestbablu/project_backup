...
public function Manage_Tour_Data()
{
		  ...
		  {
			$url = site_url('/tour/Tour/manage_tour?tour_id='.$_GET['tour_id'].'&user='.$_GET['user'].'&date='.$_GET['date'].'&contact_id='.$_GET['contact_id'].'&sales_person='.$_GET['sales_person'].'&priority='.$_GET['priority'].'&source='.$_GET['source'].'&lead_status='.$_GET['lead_status'].'&state='.$_GET['state'].'&filter='.$_GET['filter'].'&entries='.$_GET['entries']);
		  }
		  else
		  {
			$url = site_url('/tour/Tour/manage_tour?');
