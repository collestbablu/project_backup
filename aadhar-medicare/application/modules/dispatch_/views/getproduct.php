		...
		//document.getElementById("spid").value=tid;
		  ...
		  function abc(pt,pr,tid,q,u){
						...
						var pid=pt.split("^");
		  				var pids=pid[1];
					var pidd=pid[0];
					var namesize=pidd+" "+pids;
					document.getElementById("pri_id").value=pr;
				   document.getElementById("prd").value=namesize;
...
<input type="text" id="ty<?php echo $id;?>"  class="prds" value="<?php echo $arr->productname.'' ?>^<?php echo "(".$fetchsize->keyvalue.")"; ?>" name="<?php echo $id;?>"
 onFocus="abc(this.value,'<?php echo $arr->Product_id; ?>',this.id,'<?php echo $qty; ?>','<?php echo $usunit; ?>')"
 onClick="abc(this.value,'<?php echo $arr->Product_id; ?>',this.id,'<?php echo $qty; ?>','<?php echo $usunit; ?>')" style="width:240px;border:1px solid;" tabindex="-1"  readonly >
