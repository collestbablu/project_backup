...
public function manage_qc(){
	...
	$data =  $this->manage_qcJoin();
	$this->load->view('manage-qc',$data);
	}
	else
	{
	redirect('index');
...
function manage_qcJoin(){
  		$data['result'] = "";
	////Pagination start ///
      $table_name  = 'tbl_tailor';
	  $url        = site_url('/qc/manage_qc?');
	  $sgmnt      = "4";
	  $showEntries= 10;
      ...
      if($_GET['entries']!=""){
         ...
         $url     = site_url('/qc/manage_qc?entries='.$_GET['entries']);
      }
	 ...
	 if($this->input->get('filter') == 'filter')   ////filter start ////
        	$data['result']       = $this->model_qc->filterQcList($pagination['per_page'],$pagination['page'],$this->input->get());
          	else	
    		$data['result'] = $this->model_qc->getQc($pagination['per_page'],$pagination['page']);
...
public function insert_quality_check(){
@extract($_POST);
...
$status='Quality Check';
	...
	$data = array(
			...
			if($qc_id==''){					
				...
				$lastid=$qc_id;
				$this->db->query("update $table_name set qty=qty+'$qty', date='$date' where qc_id='$lastid'");	
