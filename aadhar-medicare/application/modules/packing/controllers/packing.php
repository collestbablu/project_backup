...
public function manage_packing(){
	...
	//$data['result'] = $this->model_qc->getQc();
	$data =  $this->manage_packingJoin();
...
function manage_packingJoin(){
	...
	////Pagination start ///
	 ...
	 if($this->input->get('filter') == 'filter')   ////filter start ////
        	$data['result']       = $this->model_packing->filterPackingList($pagination['per_page'],$pagination['page'],$this->input->get());
          	else	
...
public function reject_packing(){
	if($this->session->userdata('is_logged_in')){
		$this->load->view('reject-packing');
	}
	else
	{
	redirect('index');
...
$status='packing';
	...
	$data = array(
			...
			$this->packed_qty_out($finishProduct, $qty);
			$this->packed_Fgoods_qty_in($finishProduct, $qty);
...
$table_name_log ='tbl_production_log';
$pri_col_log ='production_log_id';
$status='Reject Packing';


				$this->db->query("update $table_name set reject_qty=reject_qty+'$qty', reject_date='$date' where qc_id='$qc_id'");	
...
public function packed_qty_out($finishProductId,$qty)
	{
	//echo $qty;die;
			$queryfetchtrmp=$this->db->query("select * from tbl_template_hdr where product_id='$finishProductId'");	
			$gettempFetch=$queryfetchtrmp->row();
			
			$queryfetch=$this->db->query("select * from tbl_template_dtl where templatehdr='$gettempFetch->templateid'");	
				foreach($queryfetch->result() as $fetchqrow){
					...
					//$rowFetch=$queryfetch11->row();
					//echo $fetchqrow->product_id.",";
				}		
	
	}


public function packed_Fgoods_qty_in($finishProductId,$qty)
	{
	//echo $qty;die;
			//$queryfetch=$this->db->query("select * from tbl_production_hdr where productionid='$production_id'");	
				//$fetchqrow=$queryfetch->row();
					//$Prod_id=$fetchqrow->product_id;
					//$mainqty=$qty;
					//$actual_out=$totqty*$mainqty;
					
					$this->db->query("update tbl_product_stock set quantity=quantity+'$qty' where Product_id='$finishProductId' and type='14'");
					//$rowFetch=$queryfetch11->row();
					//echo $fetchqrow->product_id.",";
