...
function manage_templateJoin(){
  		$data['result'] = "";
	////Pagination start ///
      $table_name  = 'tbl_template_hdr';
	  $url        = site_url('/template/manage_template?');
	  $sgmnt      = "4";
	  $showEntries= 10;
      ...
      if($_GET['entries']!=""){
         ...
         $url     = site_url('/template/manage_template?entries='.$_GET['entries']);
      }
	 ...
	 if($this->input->get('filter') == 'filter')   ////filter start ////
        	$data['result']       = $this->model_template->filterTemplateList($pagination['per_page'],$pagination['page'],$this->input->get());
          	else	
    		$data['result'] = $this->model_template->getTemplate($pagination['per_page'],$pagination['page']);
