...

    {
	  ...
	  alert("Your broweser doesnot support ajax");
		...
		//document.getElementById("spid").value=tid;
		  ...
		  function abc(pt,pr,tid,q,u, type){
...
for(var i = 0; i < cuisines.length; i++) {
    ...
    sel.appendChild(opt);
				   ...
				   document.getElementById("prd").value=pidd;
					...
					document.getElementById("lpr").innerHTML=pr;
					document.getElementById("lph").value=pr;
					document.getElementById("typeee").value=type;
					document.getElementById("spid").value=tid;
...
<?php
 ...
 if($con1!="")
 {
  ...
  $sel=$this->db->query("select * from tbl_product_stock where type!='14' and  productname like '%$con1%'");
...
$sqltype=$this->db->query("select * from tbl_master_data where serial_number='$arr->type'");
$fetchtype=$sqltype->row();					
...
<input type="text" id="ty<?php echo $id;?>"  class="prds form-control" value="<?php echo $arr->productname.'' ?>^<?php echo $arr->Product_id; ?>" name="<?php echo $id;?>"
 onFocus="abc(this.value,'<?php echo $arr->unitprice_purchase; ?>',this.id,'<?php echo $qty; ?>','<?php echo $usunit; ?>','<?php echo $fetchtype->keyvalue; ?>')"
 onClick="abc(this.value,'<?php echo $arr->unitprice_purchase; ?>','<?php echo $arr->unitprice_purchase; ?>',this.id,'<?php echo $qty; ?>','<?php echo $fetchtype->keyvalue; ?>','<?php echo $usunit; ?>')" style="width:240px;border:1px solid;" tabindex="-1"  readonly >
