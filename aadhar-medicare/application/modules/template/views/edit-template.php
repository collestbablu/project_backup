...
if($_GET['id']!='' or $_GET['view']!=''){
	$query=$this->db->query("select * from tbl_template_hdr where templateid='$id' or templateid='".$_GET['view']."'");	
	$fetchq=$query->row();
}

?>
<form id="f1" name="f1" method="POST" action="updateTemplate" onSubmit="return checkKeyPressed(a)" enctype="multipart/form-data">
<!-- Main content -->
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						<div class="panel-heading clearfix">
							<h4 class="panel-title"><strong>Update Template</strong></h4>
...
<select name="product_id"  required id="contact_id_copy" class="form-control" onChange="document.getElementsByName('contactid')[0].value=this.value;"   <?php if(@$_GET['view']!=''){ ?> disabled="disabled" <?php }?>>
		...
		?>
			...
			<?php if($contRow->size!=''){echo $contRow->productname.'   ( '.$psize->keyvalue .')' ;}else{echo $contRow->productname;} ?>
			</option>
			<?php } ?>
	</select>
</div>
...
<td align="center" style="width: 11%;"><input type="text" name="typeeeee[]" id="typeeeee<?php echo $z;?>" value="<?php echo $gettype->keyvalue;?>" readonly="" style="text-align: center; width: 100%; border:hidden;"></td>
...
function editselectrow(d,r) 
{
	...
	id=nn;
		...
		var typep=document.getElementById("typeeeee"+id).value;
	...
	var cell=cell+indexcell;
        cell = row.insertCell(indexcell);
				...
				var typtttt = document.createElement("input");
							...
							typtttt.border ="0";
							typtttt.value=type;	    
							typtttt.name ='typeeeee[]';
							typtttt.id='typeeeee'+rid;
