...
?>
<form id="f1" name="f1" method="POST" action="insertTemplate" onSubmit="return checkKeyPressed(a)" enctype="multipart/form-data">
<!-- Main content -->
  ...
  <script>tinymce.init({ selector:'#tem' });</script>
	...
	<div class="main-content">
		...
		<ol class="breadcrumb breadcrumb-2"> 
				...
				<li class="active"><strong><a href="#">Manage Templete</a></strong></li> 
				<div class="pull-right">
				<a class="btn btn-sm" href="<?=base_url();?>template/manage_template">Manage Template </a>
				</div>
			</ol>
		<?php }?>
			<div class="row">
				<div class="col-lg-12">
					<div class="heading">
							<h4 class="panel-title"><strong>Add Template</strong></h4>
...
<div class="field">
	<select name="product_id"  required id="contact_id_copy" class="form-control" onChange="document.getElementsByName('contactid')[0].value=this.value;"   <?php if(@$_GET['view']!=''){ ?> disabled="disabled" <?php }?>>
		...
		<?php
		$contQuery=$this->db->query("select * from tbl_product_stock where status='A' and type='14' and templateid='0'");
		foreach($contQuery->result() as $contRow)
		{
		$size=$this->db->query("select *from tbl_master_data where serial_number='$contRow->size'");
		$psize=$size->row();
		
		?>
			<option value="<?php echo $contRow->Product_id; ?>">
			<?php if($contRow->size!=''){echo $contRow->productname.'   ( '.$psize->keyvalue .')' ;}else{echo $contRow->productname;} ?></option>
		<?php }  ?>
	</select>
</div>
...
<td style="width:9%;"><div align="center"><u>Type</u></div></td>
...
<th>&nbsp;</th>
<th><input class="btn btn-sm" type="button" value="SAVE"   id="sv1" onclick="fsv(this)" >&nbsp;<a href="<?=base_url();?>template/manage_template" class="btn btn-secondary  btn-sm">Cancel</a></th></th>
</tr>
</tbody>
</table>
</div>
...
//////////////////////////////////////////////////////////////
 ...
 function adda()
		  { 
				...
				var type=document.getElementById("typeee").value;
	...
	var cell=cell+indexcell;
        cell = row.insertCell(indexcell);
				cell.style.width="3%";
				cell.align="center";
				var typtttt = document.createElement("input");
							typtttt.type="text";
							typtttt.border ="0";
							typtttt.value=type;	    
							typtttt.name ='typeett[]';
							typtttt.id='typeett'+rid;
							typtttt.readOnly = true;
							typtttt.style="text-align:center";
							typtttt.style.width="100%";
							typtttt.style.border="hidden"; 
							cell.appendChild(typtttt);
...
var pd=document.getElementById("pd"+id).value;
		...
		var typep=document.getElementById("typeett"+id).value;
...
document.getElementById("typeee").value=typep;
