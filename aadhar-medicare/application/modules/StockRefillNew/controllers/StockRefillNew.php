...
public function manage_stock_refill(){
	...
	$this->load->view('manage-stock-refill',$data);
}
else{
	redirect('index');
	}
...
function manage_stockRefillJoin(){
	...
	////Pagination start ///
	  ...
	  $url        = site_url('/StockRefillNew/manage_stock_refill?');
	  $sgmnt      = "4";
	  $showEntries= 10;
      ...
      if($_GET['entries']!=""){
         ...
         $url     = site_url('/StockRefillNew/manage_stock_refill?entries='.$_GET['entries']);
      }
