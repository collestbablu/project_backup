...
public function manage_dispatch(){
	...
	$data =  $this->manage_dispatchJoin();
	$this->load->view('Dispatch/manage-dispatch',$data);
	}
	else
	{
	redirect('index');
...
function manage_dispatchJoin(){
  		$data['result'] = "";
	////Pagination start ///
      $table_name  = 'tbl_dispatch_hdr';
	  $url        = site_url('/Authorization/dispatch/manage_dispatch?');
	  $sgmnt      = "4";
	  $showEntries= 10;
      ...
      if($_GET['entries']!=""){
         ...
         $url     = site_url('/Authorization/dispatch/manage_dispatch?entries='.$_GET['entries']);
      }
	 ...
	 if($this->input->get('filter') == 'filter')   ////filter start ////
        	$data['result']       = $this->model_dispatch->filterDispatchList($pagination['per_page'],$pagination['page'],$this->input->get());
          	else	
    		$data['result'] = $this->model_dispatch->getdipatch($pagination['per_page'],$pagination['page']);
...
public function edit_dispatch(){
	if($this->session->userdata('is_logged_in')){
		$this->load->view('Dispatch/edit-dispatch');
	}
	else
	{
	redirect('index');
...
//=======================
public function approve_dispatch(){
@extract($_POST);
...
for($i=0; $i<=$rows; $i++)
	...
	$this->db->query("update tbl_dispatch_hdr set dispatched_status='Dispatched' where dispatch_id='$dispatch_id' ");
