...
<a href="<?=base_url();?>PaymentReceived/payment_amount_invoice?id=<?php echo $_GET['id']?>&&contact_id=<?php echo $_GET['contact_id'];?>" class="btn btn-sm" target="_blank">Record Payment</a><button class="btn btn-sm" type="button" onClick="openpopup('<?=base_url();?>salesorder/SalesOrder/edit_sales_order',1200,500,'id',<?php echo $_GET['id'];?>)">Edit Invoice</button><button class="btn btn-primary" style="display:none" type="button" onClick="openpopup('<?=base_url();?>salesorder/SalesOrder/salesOrder_details_mail',1200,500,'id',<?php echo $_GET['id'];?>)">Mail Invoice</button></div>
</div>
</div><!--row-->
<br>
<?php 
...
<td><?php echo  $invoiceFetch->qty;?>(<?php echo $pfetch1->keyvalue ;?>)</td>
