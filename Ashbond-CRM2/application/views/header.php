...
$data=$this->db->query("select * from tbl_contact_m C, tbl_leads L where L.lead_id = '$rows->lead_id' AND L.contact_id = C.contact_id ");
$getData=$data->row();
$customer=$getData->first_name;

$data2=$this->db->query("select * from tbl_contact_m C, tbl_leads L where L.lead_id = '$rows->lead_id' AND L.sales_person_id = C.contact_id ");
$getData2=$data2->row();
$sales_person=$getData2->first_name;

$name=$customer."_".$sales_person;
?>
<li class="unread notification-success"><a href="#" data-toggle="modal" onClick="ViewActivityData('<?php echo $rows->lead_id;?>');" data-target="#modal-1"><i style="display:none;" class="icon-user-add pull-right"></i><span class="block-line strong">Next Action (<?php echo $rowFetch->keyvalue;?>)  <?=$name?></span><span style="display:none" class="block-line small">30 seconds ago </span></a></li>
<?php $i++;}?>
...
$.ajax({   
					...
					{   
						document.getElementById("activity_details").innerHTML = data;
					}   
				});
