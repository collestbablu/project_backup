...
public function addactivity(){
	if($this->session->userdata('is_logged_in')){
	$data['ID'] = $_GET['ID'];
		$this->load->view('crm/add-activity',$data);
	}
	else
	{
	redirect('index');
...
function update_activity_log_data() 
{
	...
	//echo "UPDATE tbl_activity_log SET communication='$com', plan_nxt_activity='$plan' WHERE activity_log_id=$id";	
	$this->db->query("UPDATE tbl_activity_log SET communication='$com', plan_nxt_activity='$plan' WHERE activity_log_id='$id'");
				
}
function delete_activity_log_data() {
	...
	$this->load->model('Model_admin_login');
		...
		$abc=$this->db->query("select * from tbl_activity_log where activity_log_id = '$id' ");
		$getAbc=$abc->row();
		
		$this->db->query("DELETE FROM tbl_activity WHERE lead_id = '".$getAbc->lead_id."' AND nxt_action_date = '".$getAbc->nxt_action_date."' ");
