...
public function Manage_Docket_Data(){


  	  $data['result'] = "";
	  ////Pagination start ///
      $table_name  = 'tbl_docket_entry';
	  $url        = site_url('/master/Account/manage_delivery?');
	  $sgmnt      = "4";
	  $showEntries= 10;
      $totalData  = $this->model_master->count_docket($table_name,'A',$this->input->get());
      //$showEntries= $_GET['entries']?$_GET['entries']:'12';
      if($_GET['entries']!=""){
         $showEntries = $_GET['entries'];
         $url     = site_url('/master/Account/manage_delivery?entries='.$_GET['entries']);
      }
     $pagination   = $this->ciPagination($url,$totalData,$sgmnt,$showEntries);
	 ...
	 if($this->input->get('filter') == 'filter')   ////filter start ////
        	$data['result']       = $this->model_master->filterDocketList($pagination['per_page'],$pagination['page'],$this->input->get());
          	else	
    		$data['result'] = $this->model_master->get_Docket_delivery($pagination['per_page'],$pagination['page']);
...
public function Manage_Docket_Entry(){

  	  $data['result'] = "";
	  ////Pagination start ///
      $table_name  = 'tbl_docket_entry';
	  $url        = site_url('/master/Account/manage_docket?');
	  $sgmnt      = "4";
	  $showEntries= 10;
      $totalData  = $this->model_master->count_docket_entry($table_name,'A',$this->input->get());
      //$showEntries= $_GET['entries']?$_GET['entries']:'12';
      if($_GET['entries']!=""){
         $showEntries = $_GET['entries'];
         $url     = site_url('/master/Account/manage_docket?entries='.$_GET['entries']);
      }
     $pagination   = $this->ciPagination($url,$totalData,$sgmnt,$showEntries);
	 ...
	 if($this->input->get('filter') == 'filter')   ////filter start ////
        	$data['result'] = $this->model_master->filterDocketEntry($pagination['per_page'],$pagination['page'],$this->input->get());
          	else	
    		$data['result'] = $this->model_master->get_Docket_Entry($pagination['per_page'],$pagination['page']);
...
echo "00"."^".$getContact->fov."^".$getContact->docket_charge."^".$getContact->cod_charge."^".$getContact->fuel_charge."^"."00"."^".$getContact->goods."^".$getContact->min_weight_surface."^".$getDtlContact->otherCharge."^".$getDtlContact->odaCharge;
}

if($_GET['type']=='Dedicated Vehicle')
{
$contactQuery=$this->db->query("select *from tbl_contact_m where contact_id='".$_GET['contId']."'  ");
$getContact=$contactQuery->row();
...
public function docket_list()
    {
         ...
         */
		 $res=$this->db->query("select *from tbl_docket_entry where status='A' and booked_status='Booked' or booked_status='Intransit' or booked_status='OFD'");
...
public function docket_list_deliver()
    {
        ...
        $res = $this -> db
           ...
           -> get('tbl_docket_entry');
         */
		 $res=$this->db->query("select * from tbl_docket_entry where status='A' and booked_status='Delivered' order by id desc  limit 100 ");
...
{
	$table_name='tbl_docket_entry';
$docketQuery=$this->db->query("select *from $table_name where docket_no='".$_GET['type']."'");
		$cntRow=$docketQuery->num_rows();
		if($cntRow>0)
		{
		echo "Docket Number Exits.Please Enter another docket Number";
		}
...
public function insert_docket()
{
		...
		@extract($_POST);
	$docketQuery=$this->db->query("select *from tbl_docket_entry where docket_no='$docket_no'");
		$cnt=$docketQuery->num_rows();
		if($cnt>0){
			$this->session->set_flashdata('message', 'Docket Number Exits.Please Enter another docket Number');
			redirect('master/Account/add_docket');
		}
		else
		{
...
$this->email->initialize(array(
  ...
  'mailtype' => 'html',
		...
		//$list = array($connor);
		//$list_consi=array($conee);


		//$this->load->library('email', $config);
		$this->email->from('info@asnlog.com',"ASNLOG");
		$this->email->to($connor);
		$this->email->cc($conee);
		//$this->email->cc("cs@asnlog.com");
...
$template = $this->load->view('Account/mail-page-booked-status', $dataaa, true);
		...
		//$this->load->library('email', $config);
		//$this->email->from('info@asnlog.com');
...
public function update_docket_delivert_status(){
		...
		if($booked_status[$i]=="Intransit" or $booked_status[$i]=="Booked" or $booked_status[$i]=="OFD" or $booked_status[$i]=='Not Delivered Return'){
			$this->db->query("update tbl_docket_entry set booked_status='$booked_status[$i]',shipment_delivered_on='$shipment_delivered_on[$i]',shipment_delivered_on_time='$shipment_delivered_on_time[$i]',remarkss='$remarks[$i]' where id='$cid[$i]'");
			
		}else{
			
			$this->db->query("update tbl_docket_entry set booked_status='$booked_status[$i]',shipment_delivered_on='$shipment_delivered_on[$i]',shipment_delivered_on_time='$shipment_delivered_on_time[$i]',remarkss='$remarks[$i]' where id='$cid[$i]'");
			
		$qry=$this->db->query("select * from tbl_docket_entry where id='$cid[$i]'");
		$fetchdata=$qry->row();
		$connor=$fetchdata->consignor_email_id;
		$conee=$fetchdata->consignee_email_id;
		$dataaa=array( 'id' => $fetchdata->id);
...
public function delivery_mail_edd_fail(){
		...
		if($upstatus=="Intransit" or $upstatus=="OFD" or $upstatus=='Not Delivered Return'){
			...
			$this->db->query("update tbl_docket_entry set booked_status='Intransit',shipment_delivered_on='$shipment_delivered_on',shipment_delivered_on_time='$shipment_delivered_on_time',remarkss='$remarkss' where id='$upstatus_id'");
...
$this->email->initialize(array(
  ...
  'mailtype' => 'html',
		...
		if ($this->email->send()) {
				...
				redirect('master/Account/manage_edd_fail');
		} else {
			redirect('master/Account/manage_edd_fail');
		}
...
public function update_edd_failed(){
  		@extract($_POST);
		 $cnt=count($cid);
		...
		$this->db->query("update tbl_docket_entry set booked_status='Intransit' where id='$cid[$i]'");
		$qry=$this->db->query("select * from tbl_docket_entry where id='$cid[$i]'");
		$fetchdata=$qry->row();
		$connor=$fetchdata->consignor_email_id;
		$conee=$fetchdata->consignee_email_id;
...
public function delivery_status(){
		...
		redirect('master/Account/manage_delivery');
		
}

public function update_not_return(){
  		@extract($_POST);
		...
		redirect('master/Account/manage_not_delivery_or_return');
		
}


public function check_my_mail()
{
$this->load->library('email');
$this->email->initialize(array(
  'protocol' => 'smtp',
...
$this->email->from('info@asnlog.com', 'Your Name');
$this->email->to('collestbablu@gmail.com,bablu.kkhan@gmail.com');
...
{
$this->load->view("Account/add-docketss.php");
}
