...
$contactDtlQuery=$this->db->query("select *from tbl_contact_dtl where contact_id='".$_GET['contId']."' ");
$getDtlContact=$contactDtlQuery->row();


echo $getContact->cft_air."^".$getContact->fov."^".$getContact->docket_charge."^".$getContact->cod_charge."^".$getContact->fuel_charge."^".$getDtlContact->rateAir;
}
...
echo $getContact->cft_train."^".$getContact->fov."^".$getContact->docket_charge."^".$getContact->cod_charge."^".$getContact->fuel_charge."^".$getDtlContact->rateTrain;
}
...
echo $getContact->cft_surface."^".$getContact->fov."^".$getContact->docket_charge."^".$getContact->cod_charge."^".$getContact->fuel_charge."^".$getDtlContact->rateSurface;
}
...
public function insert_docket()
{
		...
		$this->email->to($list);
		 $this->email->cc('gauravtaneja1801@gmail.com'); 
		$this->email->subject('Regarding');
...
public function delivery_mail(){
		...
		$ex=explode("^",$data);
		 $upstatus=$ex[0];
		 $upstatus_id=$ex[1];
		if($upstatus=="Intransit" or $upstatus=="OFD"){
			$this->db->query("update tbl_docket_entry set booked_status='$upstatus' where id='$upstatus_id'");
			redirect('master/Account/manage_docket');
		}else{
