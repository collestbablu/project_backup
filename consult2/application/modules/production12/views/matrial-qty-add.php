...
<?php 
 ...
 $Que55=$this->db->query("select * from cybercodex_stock_in_hdr where process='$process' and serial_number='$serial_number' and statusf='out'");
 //$rowq5 = $Que55->row();
 $sumc='';
 foreach($Que55->result() as $rowq5){
 
$sumc[]=$rowq5->contact_id;
$sumv[]=$rowq5->price_rate;
 }

@$imp_contact1 = implode(',',$sumc);
@$imp_price1= implode(',',$sumv);
 
$imp_contact = explode(',',$imp_contact1);
$imp_price= explode(',',$imp_price1);
...
//echo $vender_input[$m].$imp_contact[$m];

// print_r($imp_contact);

 @$key = array_search($vender_input[$m],$imp_contact,true);

//if($key=='true'){ echo $imp_price[$key];}
...
<input type="text" name="vender_input[]" id="" value="<?php if($imp_contact[$key]==$vender_input[$m] || $key!='' ){ echo $imp_price[$key];}else{ echo '0';}?>" class="col-xs-2" <?php if($countlen==1 || $countlen==2){?>style="width: 27.666667%;"<?php }else{?> class="col-xs-2"<?php }?> style="float:left; margin-top:5%;margin-left:-8%;"  placeholder="price"/>
	...
	?>				
							<tr>
						      ...
						      <?php echo $fetchQ->productname;
								...
								<td><?php 
									$Que55=$this->db->query("select * from cybercodex_stock_in_hdr where statusf='in'");
							   
							   	 foreach($Que55->result() as $fetQ55){
		 if($fetQ55->stockin_id!=''){
		 
		 $stockin_id1155[]=$fetQ55->stockin_id;
		 
			}
			}	
			
			if($stockin_id1155!=''){	
		$stockin_val = implode(',',array_unique($stockin_id1155));
		
		@$que_qty = "select sum(qntyt) as qty_sum11  from cybercodex_stock_in_dtl where product_id='$fetch->product_id' and finish_goods='$fetch->finish_goods' and process='$process' and stockin_id NOT in($stockin_val)";
		}else{
		
		@$que_qty = "select sum(qntyt) as qty_sum11  from cybercodex_stock_in_dtl where product_id='$fetch->product_id' and finish_goods='$fetch->finish_goods' and process='$process'";
		}					
			 ...
			 $total_qty1=$total_qty+$Qu_qty1s->qty_sum11;	
						...
						$quqty = "select * from cybercodex_production_dtl where product_id='$fetch->product_id' and finish_goods='$fetch->finish_goods'";
							...
							?>
							  ...
							  <input type="text" value="<?php echo $fetch->qnty*$mod_sql12->qty-$total_qty1;?>" name="Input2" id="rqty,<?php echo $i; ?>" />
							  <?php } ?></td> 
