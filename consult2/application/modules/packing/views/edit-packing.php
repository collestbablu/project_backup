...
<?php if($cntData>0){?>
 ...
 <tr>
<th>Date</th><th>Finish Goods</th><th>Unit</th><th>Total Qty</th><th>Req. Qty</th><th>Enter Qty</th><th style="display:none">Status</th>
</tr> 
    ...
    <?php
	$proQ=$this->db->query("select SUM(quantity) as quan from tbl_production_log where lot_no='".$_GET['id']."' and stage='Packing'");
	$getQt=$proQ->row();
	?>
    
	
    <td><input type="text" name="comQty111"  value="<?php echo $line->quantity-$line->rejection_qty; ?>" readonly="readonly"></td>
    <td><input type="text" name="comQty111" readonly="readonly"  value="<?php echo $line->quantity-$getQt->quan; ?>"></td>
    
    <td><input type="text" name="comQty"  value=""></td>
    
	<td style="display:none"><input type="text" name="comQty1" readonly value="<?php echo $getqueryPackingHdr->status; ?>" /></td>
