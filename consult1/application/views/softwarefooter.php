...
</script><div class="loader1" id=></div>
...
$(document).ready(function(){
	...
	jQuery('.delete_all_product').on('click', function(e) { 
		var allVals = [];  
		$(".sub_chk:checked").each(function() {  
			allVals.push($(this).attr('data-id'));
		});  
		...
		{  
			...
			//alert(join_selected_values);
				$.ajax({   
					...
					type: "POST",  
					url: "product_delete_multiple_three_table_data",  
					cache:false,  
					data: "idp="+join_selected_values+"&table_name="+table_name+"&pri_col="+pri_col,  
					success: function(response)  
					{   
						$("#loading").hide();  
						$("#msgdiv").html(response);
