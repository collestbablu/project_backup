...
<?php
 ...
 if($con1!="")
 {
  ...
  $sel=$this->db->query("select * from tbl_product_stock where Product_typeid='row_material' and  productname like '%$con1%' or sku_no like '%$con1%'");
...
<input type="text" id="ty<?php echo $id;?>"  class="prds" value="<?php echo $arr->productname.'' ?>^<?php echo $arr->Product_id; ?>" name="<?php echo $id;?>"
 onFocus="abc(this.value,'<?php echo $arr->unitprice_purchase; ?>',this.id,'<?php echo $qty; ?>','<?php echo $usunitid."^".$usunit; ?>')"
 onClick="abc(this.value,'<?php echo $arr->unitprice_purchase; ?>',this.id,'<?php echo $qty; ?>','<?php echo  $usunit; ?>')" style="width:100%;border:none;border-bottom:1px solid;border-left:1px solid;" tabindex="-1"  readonly >
