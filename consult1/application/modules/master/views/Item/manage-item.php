...
<div class="blogroll">
<div id="">
		<a type="button" class="btn btn-primary pull-right delete_all_product">Delete Selected</a>
...
<?php }?>
										...
										echo "Finish Goods"; }elseif($line->Product_typeid=='row_material'){ echo "Raw Material"; }
										elseif($line->Product_typeid=='semi_finish_goods'){ echo "Semi Finish Goods";}
										else{}?></td>
										<td>
										<?php
...
$pri_col_serial='product_id';
$table_name_log='tbl_product_serial_log';
$pri_col_log='product_id';
?>
<img src="<?php echo base_url();?>/assets/images/delete.png" id="<?php echo $line->Product_id."^".$table_name."^".$pri_col."^".$table_name_serial."^".$pri_col_serial."^".$table_name_log."^".$pri_col_log ; ?>" class="delbutton icon"  alt="" border="0"  title="Delete" />
...
<?php
	...
	?>
<input type="text" style="display:none;" id="table_name" value="tbl_product_stock^tbl_product_serial^tbl_product_serial_log">  
<input type="text" style="display:none;" id="pri_col" value="Product_id^product_id^product_id">  
</table>
...
//alert(info);
 ...
 $.ajax({
   type: "GET",
   url: "delete_multiple_three_table_data",
   data: info,
   success: function(){
