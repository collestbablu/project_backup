...
//select id of category
 $catId=$this->db->query("select *from tbl_prodcatg_m where categoryName='".$getData[2]."'");
 $catRow=$catId->row();

 //select id of category type
 $catIdType=$this->db->query("select *from tbl_producttype_m where categoryName='".$getData[3]."'");
 $catRowType=$catIdType->row();

 
//select id of unit id
 $unitId=$this->db->query("select *from tbl_master_data where keyvalue='".$getData[5]."'");
 $unitRow=$unitId->row();
	*/         
if($getData[0]!='Item Code')
{
	           
			   $this->db->query("insert into tbl_product_stock set sku_no='".$getData[0]."',productname='".$getData[1]."',category='165',usageunit='13',Product_typeid='row_material',comp_id='".$this->session->userdata('comp_id')."'");
