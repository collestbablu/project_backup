...
class defleshing extends my_controller {
...
public function updateDefleshing(){

	if($this->session->userdata('is_logged_in')){
		$this->load->view('editDefleshing');
}
	else
	{
	redirect('/master');
...
public function manage_defleshing(){

	if($this->session->userdata('is_logged_in')){
		$this->load->view('manage-defleshing');
}
...
$data = array(
			...
			'bom_id' => $bom_id[$i],
					...
					'rejection_qty' => $rejQty[$i],
					'stage' => 'QA',
					'rejection_status' => 'Rejection Done',
					'date' => $cdate,
					'maker_id' => $this->session->userdata('user_id'),
					'maker_date' => date('y-m-d'),
					'status' => 'A',
...
$this->db->query("update tbl_wip_log set rejection_status='Rejection Done' where wip_log_id='$wip_log_id[$i]'");
}
...
$table_name='tbl_defleshing_log';
$table_name_1='tbl_defleshing_hdr';
...
$data = array(
		...
		$lotQuery=$this->db->query("select *from tbl_defleshing_hdr where lot_no='".$_GET['lot_no']."'");
