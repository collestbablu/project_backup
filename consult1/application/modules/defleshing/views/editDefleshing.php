...
$hdrQuery=$this->db->query("select *from tbl_wip_hdr where wip_hdr_id='".$_GET['id']."'");
$getHdr=$hdrQuery->row();
?>

<h1>Update Defleshing</h1>
...
<th colspan="6">Update Defleshing</th>        
</tr>
</thead>
 ...
 Product Name:</td>     
	...
	<td><select name="product_name" id="p_id" <?php if(@$_GET['id']!=''){ ?>  <?php }?> required>
				...
				<?php 
							...
							$Query=$this->db->query("select * from tbl_product_stock where Product_id='$getHdr->product_name' ");
...
<td class="text-r"><star>*</star>
Lot No:</td> 			
<td><input type="text" name="lot_no" id="lot_no" value="<?php echo $getHdr->lot_no; ?>" readonly required/></td>	
</tr>        
<tr>
<td style="display:none" class="text-r"><star>*</star>
Machine Name:</td> 			
<td style="display:none"><select name="machine_name" id="machine_id" <?php if(@$_GET['id']!=''){ ?>  <?php }?> >
...
<td class="text-r"><star>*</star>
Hours:</td> 			
<td><input type="number" step="any" min="1" name="hours" id="hours" />
 </td>
...
<?php
$cntQty=$this->db->query("select SUM(quantity) as qty,SUM(rejection_qty) as rejqty from tbl_defleshing_log where bom_id='".$_GET['id']."'");
$cntfetch=$cntQty->row();



?>
	<td ><input type="number" value="<?php echo ($getHdr->quantity-$cntfetch->qty)?>" id="reqQty" readonly=""  /><br /><input type="number" value="<?php echo $cntfetch->qty;?>" readonly=""  /></td>	
<td class="text-r">Enter Qty</td>
<td ><input type="number" name="newqty" id="newqty" onchange="scrapTot();" value="" placeholder="Enter Qty" /><input style="display:none" type="number" id="scrap" value="" placeholder="Runner Qty" /><br /><input type="number" id="rejection" value="" placeholder="Rejection Qty" /><input style="display:none" type="number" step="any" id="lumbs" value="" placeholder="Lumps Qty" /><br /><input type="number"  step="any" id="act_hour" value="" placeholder="Hours" readonly="" style="display:none" /></td>
...
<th>Item Name</th><th>Unit</th><th>Quantity</th><th>Gross Weight</th><th>Net Weight</th><th>Runner Weight</th>
	...
	<td>
	<input type="text" name="gross_weight[]"  readonly value="<?php echo $line->gross_weight;?>">
	</td>
	<td><input type="text" name="net_weight[]" readonly value="<?php echo $line->net_weight; ?>"></td>
	<td><input type="text" name="scrap_weight[]" readonly value="<?php echo $line->scrap_weight; ?>"></td>
...
<th colspan="13">Defleshing In Progress</th>
 </tr>
 <tr>
<th>Date</th><th>Finish Goods</th><th>Unit</th><th>Shift</th><th>Supervisor</th><th>Operator</th><th>Hours</th><th>Act Hours</th><th>Total Qty</th><th>Completed</th><th>Rejection</th><th>&nbsp;</th><th>&nbsp;</th>
	 ...
	 if($Show!='')
	 {
	$wipLogQuery=$this->db->query("select *from tbl_defleshing_log where bom_id='".$_GET['id']."' and shift='$shift'");
	}
	else
	{
	$wipLogQuery=$this->db->query("select *from tbl_defleshing_log where bom_id='".$_GET['id']."' ");
	}
...
$ActSum=$ActSum+$actHour;
 ...
 <tr style="border-bottom:#fff solid 1px">
<th>&nbsp;</th><th>Total Qty</th><th><?php echo $getHdr->quantity; ?></th><th>Completed Qty</th><th><?=$CompSum;?></th><th>Rejected Qty</th><th><?=$regSum;?></th><th>Hours Taken</th>
...
window.location.reload(true);
	var strURL="<?php echo base_url();?>defleshing/insert_wip?id="+id+"&p_id="+p_id+"&machine_id="+machine_id+"&shift_1="+shift_1+"&actQty="+actQty+"&newqty="+newqty+"&scrap="+scrap+"&lot_no="+lot_no+"&rejection="+rejection+"&date="+date+"&operator="+operator+"&supervisor="+supervisor+"&hours="+hours+"&lumbs="+lumbs+"&act_hour="+act_hour;
...
//window.close();
	...
	getCycleTimeTot=(Number(newqty)*Number(varCycleTime)/60);
