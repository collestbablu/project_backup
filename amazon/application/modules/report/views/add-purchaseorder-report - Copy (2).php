...
<label>&nbsp;&nbsp; Show
<select name="DataTables_Table_0_length" aria-controls="DataTables_Table_0" id="entries"  url="<?=base_url();?>/report/Report/searchPurchaseOrder?<?='p_no='.$_GET['p_no'].'&v_name='.$_GET['v_name'].'&f_date='.$_GET['f_date'].'&t_date='.$_GET['t_date'];?> "class="form-control input-sm">
...
<tr>
	   ...
	   <th><div style="width:130px;">Storage Location</div></th>
...
<th><?php 
        ...
        <th> <button class="btn btn-xs btn-black"  type="button" data-toggle="modal" data-target="#modal-1" onclick="viewInboundOutbondDetails('<?=$sales->purchaseid;?>');"> <i class="icon-eye"></i></button>
                            </th>
...
function viewInboundOutbondDetails(viewId){

 	$.ajax({   
	    type: "POST",  
		url: "<?=base_url();?>report/view_inbound_outbond_details",  
		cache:false,  
		data: {'id':viewId},  
