...
<th>
		<?php 
		$i = 0; $mresult = array();$rrval = "";
			$sqlgroup11=$this->db->query("select * from tbl_product_mapping where product_id = '".$rows->Product_id."'");
			foreach ($sqlgroup11->result() as $fetchgroup11){
			  $productmm  = $fetchgroup11->location;	
			 //echo "select * from tbl_master_data where serial_number IN ($productmm)";
             $sqlcont    = $this->db->query("select * from tbl_master_data where serial_number IN ($productmm)");
			 $queryciont = $sqlcont->result();
            
             $mresult  = array();
            foreach ($queryciont as $dttt) {
             $mresult[] = $dttt->keyvalue;
            }
             $rrval = "";
            if(sizeof($mresult) > 0){
             $rrval =  implode(',',$mresult);
            }
            
		?>
		<p style="<?=($i%2)==0?'color:#000;':'color:red;'?>;margin: 0px !important;"><?=$rrval;?></p>	
