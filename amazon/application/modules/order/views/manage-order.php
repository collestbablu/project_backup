...
/*	.error{
	...
	<div class="main-content">
			...
			<ol class="breadcrumb breadcrumb-2"> 
                ...
                <li><a class="active">Manage  Order</a></li> 
              <!--   <li ><strong>Add Item11</strong></li> -->
			
                <div class="pull-right">
                   <a class="btn btn-sm btn-black btn-outline" href="<?=base_url();?>order/add_order"><i class="fa fa-plus" aria-hidden="true"></i>Create STN</a>
	     ...
	     <div class="row">
			 ...
			 <div class="panel panel-default">
				...
				<div class="col-sm-5">
				<label>Show 
                <select   name="DataTables_Table_0_length" url="<?=base_url();?>order/manage_order?<?='location='.$_GET['location'].'&destination='.$_GET['destination'].'&order_date='.$_GET['order_date'].'&puchase_contact='.$_GET['puchase_contact'].'&filter='.$_GET['filter'];?>" id="entries" class="">
					...
					<thead>
					 <tr>  
						...
						<tr>
						    <form method="get">
								...
								<!-- <td><input  name="category"  type="text"  class="search_box form-control input-sm" value="" /></td> -->
										<td><input  name="stn_no"  type="text"  class="search_box form-control input-sm"  value="" /></td>
						...
						foreach($result as $sales)
					    {
					    	...
					    	$arrH['orderid']          =  $sales->purchaseid;
					    	$arrH['docket_no']        =  $sales->docket_no;
					    	$arrH['vehicle_no']       =  $sales->vehicle_no;
					    	$arrH['transporter_name'] =  $sales->transporter_name;
					    	$arrH['vechicle_date']    =  $sales->vechicle_date;
					    	$arrH['orderid']          =  $sales->purchaseid;

					    	$jsondocket = json_encode($arrH,true);
						?>
                         <tr class="gradeC record">
							...
							<td>
						     ...
						     <!-- <button class="btn btn-xs btn-black delbuttonPurchase" id="<?=$sales->purchaseid."^".$table_name."^".$pri_col ; ?>" type="button"><i class="icon-trash"></i></button> -->
							 
							 <?php
							 if($sales->docket_no == ""){
							 ?>
			                 <button style="margin-top: 4px;" class="btn btn-xs btn-black" data-toggle="modal" data-target="#modal-3" onclick="docketDetails(this);" json-docket ='<?=$jsondocket;?>' type="button">Add Docket </button>
							 
							  <?php
							 }else{
							 ?>
			                 <button style="margin-top: 4px;" class="btn btn-xs btn-black" data-toggle="modal" data-target="#modal-3" onclick="docketDetails(this);" json-docket ='<?=$jsondocket;?>' type="button">View Docket </button><?php } ?>
                            </td>
						    <?php } ?>
						 </tr>
						</tbody>
             ...
             <div id="modal-3" class="modal fade" tabindex="-1" role="dialog">
					<div class="modal-dialog modal-lg">
					  <div class="modal-content" >
						<div class="modal-header">
						<button  class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h4 class="modal-title">Docket Details</h4>
						<div id="msgdata2" class="text-center " style="font-size: 15px;color: red;"></div> 
						</div>

						
                         <div class="panel-body" id ="docketDetailsData">

                  <form class="form-horizontal" role="form" id="docketForm" novalidate="novalidate">	 
					<div class="modal-body overflow">
                     <div class="form-group"> 
						<label class="col-sm-2 control-label">DOCKET NO.:</label> 
							<div class="col-sm-4"> 
							 <input type="text" class="form-control" id="docket_no" required name="docket_no" value="">
						    </div> 
					...
					<div class="form-group"> 
						...
						<div class="col-sm-4"> 				
						<input type="hidden" name="orderid" id="orderid" class="hiddenField" value="">
...
<script>
  ...
  function destinationfunction(ths){
  	 	$.ajax({   
				...
				{ 
				  // alert(data); 
				  // console.log(data);
			     $("#destination").empty().append(data).fadeIn();
				}   
		});
 ...
 function maplocation(ths){
  ...
  function mapconsigneeAddress(ths){
 	       $.ajax({   
			    type: "POST",  
...
function inbondviewlog(poid){
	...
	function docketDetails(ths){
	    var jsonVal = $(ths).attr('json-docket');
	    var button_property = $(ths).attr('property');
	    editVal = "";
	    if(jsonVal !== undefined)
	     var editVal = JSON.parse(jsonVal);

     console.log(editVal.docket_no);
	    if(editVal != ""){
          $('#docket_no').val(editVal.docket_no);
          $('#vehicle_no').val(editVal.vehicle_no);
          $('#transporter_name').val(editVal.transporter_name);
          $('#vehicle_date').val(editVal.vechicle_date);
          $('#orderid').val(editVal.orderid);
        }
    }
