...
if($con1!="")
 {
    if($prdIdd != ""){
      $qryy = "select * from tbl_product_stock S,tbl_product_mapping M where S.Product_id = M.product_id AND S.productname like '%$con3%' AND M.suplier_id = $suppliercontact AND find_in_set($company,M.location) AND S.Product_id NOT IN ($prdIdd)";
     }else{
      $qryy =  "select * from tbl_product_stock S,tbl_product_mapping M where S.Product_id = M.product_id AND S.productname like '%$con3%' AND M.suplier_id = $suppliercontact AND find_in_set($company,M.location)";
     }
