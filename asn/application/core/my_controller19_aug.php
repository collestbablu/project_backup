...
//================================*Start delete sales order ============== 
 function delete_sales_order_data() {
	...
	$this->load->model('Model_admin_login');
		...
		$table_name2 =tbl_invoice_payment;
		$pri_col2 =invoiceid;
		$table_name3 =tbl_software_log;
		$pri_col3 =id;
		
		// starts select product id and qty from sales table //
		
		$selectSalesDtl=$this->db->query("select *from $table_name1 where salesid='$id'");
	...
	for($p=0;$p<=$sizecount;$p++){
		...
		$this->delete_updata_stock($qty,$main_id,$sumqtynt);
		...
		$sales_softlog=$this->db->query("select * from tbl_software_log where log_id='$id'");
		foreach($sales_softlog->result() as $getsales_softlog){
		
		 $softlog_id=$getsales_softlog->id;
		
		$this->db->query("delete from tbl_software_log where type='Sales Order added' or type='Sales Order Updated' and log_id='$id'");
