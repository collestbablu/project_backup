...
public function docket_list()
    {
         ...
         */
		 $res=$this->db->query("select *from tbl_docket_entry where status='A' and booked_status='Booked' or booked_status='Intransit' ");
...
public function update_docket_delivert_status(){
		 ...
		 {
		 if($booked_status[$i]!=''){
			  echo $booked_status[$i];
		if($booked_status[$i]=="Intransit" or $booked_status[$i]=="OFD" or $booked_status[$i]=='Not Delivered Return'){
...
$this->email->initialize(array(
  ...
  'mailtype' => 'html',
		...
		$this->email->from('info@techvyaserp.in',"asnlog");
		$this->email->to($list);
		$this->email->to($listconsi);
...
$this->email->initialize(array(
  ...
  'mailtype' => 'html',
		...
		//$this->load->library('email', $config);
		$this->email->from('info@asnlog.com',"asm");
		$this->email->to($dataC);
		 //$this->email->bcc('collestbablu@gmail.com'); 
		 $sub='Regarding AWB-'.$fetchdata->docket_no;
		$this->email->subject($sub);
